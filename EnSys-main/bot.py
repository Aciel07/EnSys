            ...
            for item in items:
                if item['name'].lower() in preprocessed_prompt:
                    ...
                    if sentiment['compound'] > 0.05:
                        praise_response = f"Thank you for the kind words about our {item['name']}!"
                        self.add_system_message(praise_response)
                        self.save_chat_history()  # Save the chat history
                        print("Bot:", praise_response)
                        return praise_response
                    # If the sentiment is negative, treat it as criticism
                    elif sentiment['compound'] < -0.05:
                        criticism_response = f"We're sorry to hear that you're not satisfied with our {item['name']}. Could you please provide more details?"
                        self.add_system_message(criticism_response)
                        self.save_chat_history()  # Save the chat history
                        print("Bot:", criticism_response)
                        return criticism_response
