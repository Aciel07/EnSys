...
import pandas as pd
from flask import Flask, session, render_template, request, jsonify, redirect, send_file, url_for
...
def initialize_routes(app, chatbot):
    ...
    def login():
        if request.method == 'POST':
            ...
            if validate_admin(username, password):
                session['admin'] = username  # Set session variable
    ...
    def change_password():
        if request.method == 'POST':
            username = request.form['username']  # Admin username
            ...
            # Validate current password for the provided admin username
            if not validate_admin(username, current_password):
        ...
        # GET request renders the change password form
